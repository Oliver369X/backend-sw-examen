...
export const register = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const token = jwt.sign({ userId: newUser.id, correo: newUser.correo }, JWT_SECRET, { expiresIn: '1h' });
...
export const login = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const token = jwt.sign({ userId: user.id, correo: user.correo }, JWT_SECRET, { expiresIn: '1h' });
