...
// Estructura para los errores detectados
...
// Estructura para la entrada del auditor
interface AuditorInput {
  code: string;
  context?: string; // Contexto adicional sobre el código
  focusAreas?: string[]; // Áreas específicas a revisar
}

export const expertAuditor = createExpert({
  name: 'Code Auditor',
...
Para errores de Flutter específicos, proporciona el código corregido completo.`,
  ...
  async process({ code, context = '', focusAreas = [] }: AuditorInput): Promise<string> {
    try {
