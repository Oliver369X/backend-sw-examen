...
interface ImageToBackendInput {
  imageBase64: string; // Imagen en base64
  mimeType?: string; // image/png, image/jpeg, etc.
}

interface ImageToBackendOutput {
  diagramData?: {
    classes: any[];
    interfaces: any[];
    relationships: any[];
...
export const expertImageToBackend = createExpert({
...
RESPONDE ÚNICAMENTE CON JSON VÁLIDO, SIN MARKDOWN NI EXPLICACIONES.`,
  ...
  async process(input: ImageToBackendInput): Promise<ImageToBackendOutput> {
