...
import { UMLParser, UMLDiagramData, UMLClass, UMLRelationship } from '../utils/umlParser';
...
export class SpringBootTemplateEngine {
  private templatesPath: string;

  constructor() {
    this.templatesPath = path.join(__dirname, '../templates/springboot');
  }

  /**
   * Genera un proyecto Spring Boot completo desde un diagrama UML
   */
  async generateProject(umlDiagram: UMLDiagramData, config: GenerationConfig): Promise<GeneratedFile[]> {
   ...
   */
  private prepareClassData(umlClass: UMLClass, relationships: UMLRelationship[], config: GenerationConfig) {
    const classRelationships = relationships.filter(rel => 
      rel.sourceClassId === umlClass.id || rel.targetClassId === umlClass.id
    );
    ...
    return {
      ...
      attributes: attributes.map(attr => ({
        ...attr,
        javaType: UMLParser.mapUMLTypeToJava(attr.type),
      ...
      idType: UMLParser.mapUMLTypeToJava(
        attributes.find(attr => attr.name.toLowerCase() === 'id')?.type || 'Long'
      ),
   ...
   */
  private async generateEntity(classData: any, config: GenerationConfig): Promise<GeneratedFile> {
   ...
   * Genera un DTO
   */
  private async generateDTO(classData: any, config: GenerationConfig): Promise<GeneratedFile> {
   ...
   * Genera un Mapper
   */
  private async generateMapper(classData: any, config: GenerationConfig): Promise<GeneratedFile> {
   ...
   */
  private async generateRepository(classData: any, config: GenerationConfig): Promise<GeneratedFile> {
   ...
   * Genera un servicio
   */
  private async generateService(classData: any, config: GenerationConfig): Promise<GeneratedFile> {
   ...
   */
  private async generateController(classData: any, config: GenerationConfig): Promise<GeneratedFile> {
   ...
   * Genera pom.xml
   */
  private async generatePomXml(config: GenerationConfig): Promise<GeneratedFile> {
   ...
   */
  private async generateApplicationProperties(config: GenerationConfig): Promise<GeneratedFile> {
   ...
   */
  private async generateMainClass(config: GenerationConfig): Promise<GeneratedFile> {
   ...
   */
  private async generateEntityTest(classData: any, config: GenerationConfig): Promise<GeneratedFile> {
   ...
   */
  private async generateServiceTest(classData: any, config: GenerationConfig): Promise<GeneratedFile> {
   ...
   */
  private async generateSecurityConfig(config: GenerationConfig): Promise<GeneratedFile> {
   ...
   */
  private async generateCacheConfig(config: GenerationConfig): Promise<GeneratedFile> {
   ...
   */
  private async generateGlobalExceptionHandler(config: GenerationConfig): Promise<GeneratedFile> {
   ...
   */
  private generateImports(umlClass: UMLClass, relationships: any[]): string[] {
    ...
    const hasDates = umlClass.attributes.some(attr => 
   ...
   */
  private generateImportsForTests(classData: any, isServiceTest: boolean = false): string[] {
    const imports = new Set<string>();
   ...
   */
  private generateQueryMethods(classData: any): any[] {
    const methods = [];
   ...
   */
  private findClassNameById(classId: string, relationships: UMLRelationship[]): string {
   ...
   */
  private generatePropertyName(relationship: UMLRelationship, classId: string): string {
    if (relationship.name) {
      return relationship.name;
    }
    
    const targetClass = this.findClassNameById(
      relationship.sourceClassId === classId ? relationship.targetClassId : relationship.sourceClassId,
      []
    );
   ...
   */
  private generateMappedBy(relationship: UMLRelationship, classId: string): string {
   ...
   */
  private generateJoinColumn(relationship: UMLRelationship, classId: string): string {
    const targetClass = this.findClassNameById(
      relationship.sourceClassId === classId ? relationship.targetClassId : relationship.sourceClassId,
      []
    );
   ...
   */
  private generateJoinTable(relationship: UMLRelationship): string {
   ...
   */
  private generateInverseJoinColumn(relationship: UMLRelationship, classId: string): string {
    const targetClass = this.findClassNameById(
      relationship.sourceClassId === classId ? relationship.targetClassId : relationship.sourceClassId,
      []
    );
   ...
   */
  private getMaxLength(type: string): number | undefined {
   ...
   */
  private getMinValue(type: string): number | undefined {
   ...
   */
  private getMaxValue(type: string): number | undefined {
    if (type === 'Integer') return 2147483647;
    if (type === 'Long') return undefined;
    return undefined;
  }
   ...
   */
  private getColumnDefinition(type: string): string | undefined {
   ...
   */
  private hasUniqueConstraints(attributes: any[]): boolean {
   ...
   */
  private generateUniqueConstraints(attributes: any[]): any[] {
   ...
   */
  private validateGeneratedCode(code: string, fileName: string): void {
   ...
   */
  private async validateProjectCompilation(projectPath: string): Promise<boolean> {
    try {
